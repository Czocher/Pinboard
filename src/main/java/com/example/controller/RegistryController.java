...
public class RegistryController {
	...
	public String create_user(@RequestParam("email") String email, @RequestParam("login") String login,
			@RequestParam("password") String password, @RequestParam("rep_password") String password2, Model model) {
			...
			if (password.equals(password2)) {
				...
				mm.sendMail("giny21@gmail.com", user.getEmail(), "Pin Board - Account Activation",
						"Hello <br> This is your activation email for Pin Board platform <br>"
								+ "Click this link: <a href=''>link</a>"); // zmienic
																			// na
																			// link!
			} else
				return "redirect:/registry?err=-2";
		} else
			return "redirect:/registry?err=-1";
		return "redirect:/?u=1";
	}
}
